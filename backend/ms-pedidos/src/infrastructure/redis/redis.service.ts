...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      this.subClient = null;
      this.logger.warn(
        `Redis connection failed (${err.message}). Falling back to LOCAL mock in-memory Pub/Sub.`,
      );
    }
  }
  ...
  async publish(channel: string, payload: any): Promise<void> {
    ...
    if (!this.isMockMode && this.pubClient) {
      try {
        ...
        this.logger.error(
          `Error publishing to Redis: ${err.message}. Routing to in-memory fallback.`,
        );
  ...
  async subscribe(
    ...
    if (!this.isMockMode && this.subClient) {
      try {
        ...
        this.logger.error(
          `Error subscribing to Redis: ${err.message}. Routing to in-memory fallback.`,
        );
