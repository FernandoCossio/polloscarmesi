...
export class S3Service {
  ...
  constructor(private readonly configService: ConfigService) {
    this.storageType = this.configService.get<string>('aws.storageType', 'local');
    this.bucketName = this.configService.get<string>('aws.s3BucketName', 'polloscarmesi-delivery-evidence');
    ...
    if (this.storageType === 's3') {
      ...
      if (accessKeyId && secretAccessKey) {
        ...
        this.logger.warn('S3 Storage active but credentials are empty. S3 calls will fail.');
      }
    } else {
