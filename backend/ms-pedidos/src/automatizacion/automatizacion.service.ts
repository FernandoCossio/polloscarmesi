...
export class AutomatizacionService {
  ...
  async realizarCierreCaja(
    fecha?: string,
  ): Promise<{ url: string; consolidado: any }> {
    ...
    const ms1Presenciales = ms1Pedidos.filter(
      (p) => p.tipo === 'PRESENCIAL' && p.estado === 'ENTREGADO',
    );
    const totalMs1Ventas = ms1Presenciales.reduce(
    ...
    const entregados = ms2Pedidos.filter(
      (p) => p.estado === EstadoDelivery.ENTREGADO,
    );
    const cancelados = ms2Pedidos.filter(
      (p) => p.estado === EstadoDelivery.CANCELADO,
    );
    const totalMs2Ventas = entregados.reduce(
      (sum, p) => sum + Number(p.total),
      0,
    );
    ...
    const reportUrl = await this.s3Service.uploadFile(
      pdfBuffer,
      s3Key,
      'application/pdf',
    );
  ...
  private async generateReportPdf(data: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc
        .fontSize(22)
        .fillColor('#B22222')
        .text('POLLOS CARMESÍ', { align: 'center' });
      doc
        .fontSize(14)
        .fillColor('#3E2723')
        .text('Reporte Consolidado de Cierre de Caja', { align: 'center' });
      doc.moveDown(2);

      doc
        .fontSize(10)
        .fillColor('#757575')
        .text(`Fecha del Cierre: ${data.fecha}`);
      ...
      doc
        .fontSize(14)
        .fillColor('#B22222')
        .text('Ventas Presenciales (MS1)', { underline: true });
      doc.moveDown(0.5);
      doc
        .fontSize(11)
        .fillColor('#3E2723')
        .text(`Pedidos Totales: ${data.ms1.totalPedidos}`);
      doc.text(`Pedidos Entregados (Cobrados): ${data.ms1.entregados}`);
      doc
        .fontSize(12)
        .text(
          `Total Recaudado Presencial: ${data.ms1.totalVentas.toFixed(2)} Bs.`,
        );
      doc.moveDown(1.5);
      ...
      doc
        .fontSize(14)
        .fillColor('#B22222')
        .text('Ventas Delivery (MS2)', { underline: true });
      doc.moveDown(0.5);
      doc
        .fontSize(11)
        .fillColor('#3E2723')
        .text(`Pedidos Totales: ${data.ms2.totalPedidos}`);
      doc.text(`Entregas Completadas: ${data.ms2.entregados}`);
      doc.text(`Pedidos Cancelados: ${data.ms2.cancelados}`);
      doc
        .fontSize(12)
        .text(
          `Total Recaudado Delivery: ${data.ms2.totalVentas.toFixed(2)} Bs.`,
        );
      doc.moveDown(1.5);
      ...
      doc
        ...
        .fontSize(16)
        .fillColor('#B22222')
        .text(`TOTAL CONSOLIDADO DEL DÍA: ${data.totalGlobal.toFixed(2)} Bs.`, {
          align: 'right',
        });
